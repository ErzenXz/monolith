...
async function handler(request, apiKey) {
  ...
  try {
    ...
    const options = {
      ...
      thumbnails: formData.fields.thumbnails ? parseInt(formData.fields.thumbnails) : undefined,
      format: formData.fields.format || 'mp4'
    }
    ...
    return errorResponse(error.message || 'Failed to compress video', 500)
  }
}
...
async function processJob(jobId) {
  ...
  try {
    ...
    const compressionResult = await compressor.compressMedia(fileBuffer, 'video', options)
    ...
    for (const thumbnail of compressionResult.thumbnails) {
      const filename = storage.generateFilename(`thumbnail-${thumbnail.timestamp}`, thumbnail.format)
      uploadPromises.push(
